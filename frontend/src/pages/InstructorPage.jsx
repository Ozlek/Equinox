...
export default function InstructorPage() {
  ...
  const handleDeleteQuestion = async (id) => {
    ...
    try {
      await api.delete(`/accounts/instructor/questions/${id}/`);
      fetchQuestions();
    } catch (err) {
  ...
  const handleSaveQuestion = async (data) => {
    try {
      if (data.id) {
        await api.put(`/accounts/instructor/questions/${data.id}/`, data);
      } else {
        await api.post("/accounts/instructor/questions/", data);
      }
      setEditModal(null);
      fetchQuestions();
    } catch (err) {
                                ...
                                {paginatedQuestions.map((q) => (
                                  <div key={q.id} style={styles.questionRow}>
                                    <div style={{ flex: 1 }}>
                                      ...
                                      <p style={styles.qMeta}>G{q.grade_level} · Diff: {q.difficulty} · WP: {q.is_word_problem ? "Yes" : "No"}</p>
